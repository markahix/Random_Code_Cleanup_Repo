...
! GAC: allocate arrays
  ...
  write(6,*)' Finnished allocating arrays'
...
! nlast_at = The last atom to be considered
 ...
 30     continue
        write(804,*) ' done writting std_am_at_types'
 901    format(i10,a4)
 ...
 50     continue
 40     continue
        write(804,*) ' done writting atom types'
...
!
 ...
 902    format(2i10,4f10.4)
        write(804,*) ' done writting LJ parameters ',k1
!
...
!
 ...
 910    format(20i4)
         write(804,*) ' done writting number of atoms in a residue'
!
!   set up bins...
!
        do 110 k1=1,npairs
...
!       write(804,911) &
!          k2,k4,k3,k5,k1,at_id_i,at_type_i,at_id_j,at_type_j
       ! if(k2.eq.nres-1.and.k4.eq.nres) write(804,911) &
       !    k2,k4,k3,k5,k1,at_id_i,at_type_i,at_id_j,at_type_j
 150    continue
 140    continue
 130    continue
...
!     atom k3 of residue k2
          do 340 k3=1,nat_in_res(k2)

!     atom k5 of residue k4
           do 350 k5=1,nat_in_res(k4)
...
! van der Waals Eenergy
        E_vdW  = vdw_12(k1)/rd12 - vdw_6(k1)/rd6
       !if (k2 .eq. 4 .and. k4 .eq. 533)&
       !    print *, k2,k4,k3,k5,vdw_12(k1),vdw_12(k1),E_vdW
!
!  binning.....
!
        k_bin = iflag(k1)
